...
export function CheckoutPage() {
  ...
  const [form, setForm] = useState(() => ({
    ...m.location,
    ...m.draft,
    date: dates.some(d => d.value === m.draft.date) ? m.draft.date : dates[0].value,
    time: m.draft.time || '09:00–12:00'
  }));
  ...
  useEffect(() => {
    if (m.location) setForm(current => ({
      ...current,
      ...m.location
    }));
  }, [m.location?.street, m.location?.city, m.location?.postcode]);
