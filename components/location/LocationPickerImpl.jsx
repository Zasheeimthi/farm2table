...
async function findPlaces(path, signal) {
  const response = await fetch(`https://photon.komoot.io/${path}`, { signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(10000)]) : AbortSignal.timeout(10000) });
...
export function FarmRegionMap({ location }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(location)}&limit=1&lang=en`, { signal: AbortSignal.any([controller.signal, AbortSignal.timeout(10000)]) })
