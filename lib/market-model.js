/**
 * Pure, isomorphic pricing/basket model.
 * No React, no browser APIs, no framework imports - safe on server and client.
 */

export const slugify = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
export const productPath = (item) => `/product/${slugify(item.title)}`;
